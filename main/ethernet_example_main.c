...
/* LED stuff
 */
#define LED_ETH GPIO_NUM_25
#define LED_WIFI GPIO_NUM_26
#define LED_CONNECTED GPIO_NUM_32
esp_err_t init_led() {
#define SET_OUTPUT(GPIO) ESP_ERROR_CHECK(gpio_set_direction(GPIO, GPIO_MODE_OUTPUT))
    SET_OUTPUT(LED_ETH);
    SET_OUTPUT(LED_WIFI);
    SET_OUTPUT(LED_CONNECTED);
    return ESP_OK;
}

esp_err_t switch_light(gpio_num_t gpio_num, bool on) {
    ESP_ERROR_CHECK(gpio_set_level(gpio_num, on));
    return ESP_OK;
}

TaskHandle_t xBlinkHandle = NULL;
#define CREATE_BLINKING xTaskCreate(blink, "BlinkLED", 1000, (void*)1, tskIDLE_PRIORITY, &xBlinkHandle)
#define REMOVE_BLINKING vTaskDelete(xBlinkHandle)
void blink(void *pvParameters) {
    const TickType_t xDelay = 500 / portTICK_PERIOD_MS;
    bool v = true;
    while (true) {
        switch_light(LED_CONNECTED, v);
        vTaskDelay(xDelay);
        v = !v;
    }
}
                              ...
                              int32_t event_id, void *event_data)
{
    ...
    case ETHERNET_EVENT_CONNECTED:
        ...
        switch_light(LED_ETH, true);
        break;
    case ETHERNET_EVENT_DISCONNECTED:
        ESP_LOGI(TAG, "Ethernet Link Down");
        switch_light(LED_ETH, false);
        break;
...
#define EXAMPLE_ESP_WIFI_SSID      "eth2ap"
#define EXAMPLE_ESP_WIFI_PASS      "pnqeicsntartfvtsiceit"
                               ...
                               int32_t event_id, void* event_data)
{
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        REMOVE_BLINKING;
        switch_light(LED_CONNECTED, true);
    } else if (event_id == WIFI_EVENT_AP_STADISCONNECTED) {
        wifi_event_ap_stadisconnected_t* event = (wifi_event_ap_stadisconnected_t*) event_data;
        ESP_LOGI(TAG, "station "MACSTR" leave, AID=%d",
                 MAC2STR(event->mac), event->aid);
        switch_light(LED_CONNECTED, false);
        CREATE_BLINKING;
    }
}
